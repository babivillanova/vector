...
def _load_env():
    """Load .env file into os.environ (for CLI use only)."""
...
# ──────────────────────────────────────────────
def detect_present_elements(img_bytes: bytes, mime_type: str) -> Dict[str, bool]:
    """
...
# ──────────────────────────────────────────────
def detect_door_hinges(img_bytes: bytes, doors: list, mime_type: str = "image/png") -> list:
    """
...
# ──────────────────────────────────────────────
def _load_door_elements(output_dir: Path) -> list:
...
# ──────────────────────────────────────────────
# Main pipeline (callable from API or CLI)
# ──────────────────────────────────────────────
def run_pipeline(
    img_bytes: bytes,
    filename: str,
    ...
    Args:
        img_bytes: Raw image file bytes (PNG or JPEG).
        filename: Original filename (used for naming outputs).
        mime_type: MIME type of the image ("image/png" or "image/jpeg").
        output_dir: Directory for intermediate files.
        use_cache: If True, reuse cached Gemini layer images.

    Returns:
        Dict mapping output filename → file content bytes.
        Keys include: gemini_*.png, layer_*.png, *_layered.svg,
        layer_debug_overlay.png
    """
    print("=" * 60)
    ...
    for layer_name in masks:
        mask_path = output_dir / f"layer_{layer_name}.png"
        if mask_path.exists():
            outputs[f"layer_{layer_name}.png"] = mask_path.read_bytes()
    ...
    outputs[svg_filename] = layered_svg.encode("utf-8")

    # Debug overlay
    save_debug_output(masks, h, w, output_dir)
    overlay_path = output_dir / "layer_debug_overlay.png"
    if overlay_path.exists():
        outputs["layer_debug_overlay.png"] = overlay_path.read_bytes()
...
def main():
    """CLI entry point — reads file from disk, runs pipeline, writes outputs."""
    _load_env()
    ...
    mime_type = _mime_from_filename(input_path.name)
